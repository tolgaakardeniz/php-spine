...
namespace PhpProje\Library\PhpImageMagician;

use Exception;
use GdImage;
use PhpProje\Library\PhpImageMagician\PhpPsdReader\PhpPsdReader;
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	#
	{
		...
		if (!file_exists($this->fileName)) {
			if ($debug) {
				throw new Exception('Image not found. File: ' . $this->fileName);
			} else {
				return array();
			}
		};
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	#
	{
		...
		if (!file_exists($file) && !$this->checkStringStartsWith('http://', $file)) {
			if ($this->debug) {
				throw new Exception('Image not found. File: ' . $file);
			} else {
				throw new Exception();
			}
		};
...
*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-**-*-*-*-*-*-*-*-*-*-*-*-*-*/
	...
	private function GetPixelColor(GdImage &$img, $x, $y)
	# Author:     James Heinrich
	# Purpose:
	# Param in:
	# Param out:
